  ...
  const onSubmit = useCallback(
    async (data: FormData) => {
      ...
      try {
        ...
        if (visibleFields.includes('country') && data.country) fd.append('country', data.country)
        if (visibleFields.includes('phoneNumber') && data.phoneNumber)
          fd.append('phoneNumber', data.phoneNumber)
        if (visibleFields.includes('companyName') && data.companyName)
          fd.append('companyName', data.companyName)
        if (visibleFields.includes('companyPosition') && data.companyPosition)
          fd.append('companyPosition', data.companyPosition)
        if (visibleFields.includes('companyWebsite') && data.companyWebsite)
          fd.append('companyWebsite', data.companyWebsite)
        if (visibleFields.includes('presentationTopic') && data.presentationTopic)
          fd.append('presentationTopic', data.presentationTopic)
        if (visibleFields.includes('presentationSummary') && data.presentationSummary)
          fd.append('presentationSummary', data.presentationSummary)
        if (visibleFields.includes('technicalRequirements') && data.technicalRequirements)
          fd.append('technicalRequirements', data.technicalRequirements)

        // File uploads — append raw File objects
        if (data.imageUrl && data.imageUrl.length > 0) fd.append('imageUrl', data.imageUrl[0])
        if (data.companyLogoUrl && data.companyLogoUrl.length > 0)
          fd.append('companyLogoUrl', data.companyLogoUrl[0])
