...
import { registerPartner } from '@/app/(frontend)/partner-register/actions'
  ...
  const onSubmit = useCallback(
    async (data: FormData) => {
      ...
      try {
        ...
        fd.append('partnerType', partnerTypeId)

        // Optional text fields — only append if present and visible
        if (visibleFields.includes('email') && data.email) fd.append('email', data.email)
        if (visibleFields.includes('fieldOfExpertise') && data.fieldOfExpertise)
          fd.append('fieldOfExpertise', data.fieldOfExpertise)
        if (visibleFields.includes('companyWebsiteUrl') && data.companyWebsiteUrl)
          fd.append('companyWebsiteUrl', data.companyWebsiteUrl)
        if (visibleFields.includes('companyLogoUrl') && data.companyLogoUrl)
          fd.append('companyLogoUrl', data.companyLogoUrl)
        if (visibleFields.includes('companyDescription') && data.companyDescription)
          fd.append('companyDescription', data.companyDescription)
        if (visibleFields.includes('sponsorshipLevel') && data.sponsorshipLevel)
          fd.append('sponsorshipLevel', data.sponsorshipLevel)
        if (visibleFields.includes('additionalNotes') && data.additionalNotes)
          fd.append('additionalNotes', data.additionalNotes)

        // File uploads — append the raw File objects
        if (data.companyLogo && data.companyLogo.length > 0)
          fd.append('companyLogo', data.companyLogo[0])
        if (data.companyBanner && data.companyBanner.length > 0)
          fd.append('companyBanner', data.companyBanner[0])

        // Social links — serialised as JSON
        const filteredSocialLinks =
          data.socialLinks?.filter((link) => link.url.trim() !== '') ?? []
        if (visibleFields.includes('socialLinks') && filteredSocialLinks.length > 0)
          fd.append('socialLinks', JSON.stringify(filteredSocialLinks))

        setUploadProgress(60)

        // Call Payload Local API via server action
    ...
    [partnerTypeId, selectedEventId, visibleFields],
  )
        ...
        {loading && uploadProgress > 0 && uploadProgress < 100 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
              <span>
                {uploadProgress < 60 ? 'Preparing submission...' : 'Processing registration...'}
              </span>
