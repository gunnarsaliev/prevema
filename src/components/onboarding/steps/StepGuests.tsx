...
import { createParticipantTypeAction, createPartnerTypeAction } from '@/app/(frontend)/onboarding/actions'
...
export const StepGuests = ({
  ...
  const [participantFields, setParticipantFields] = useState<string[]>(['imageUrl', 'biography', 'companyName'])
  const [participantTypes, setParticipantTypes] = useState<GuestType[]>([])
  const [participantLoading, setParticipantLoading] = useState(false)

  // Partner state
  const [partnerName, setPartnerName] = useState('')
  const [partnerFields, setPartnerFields] = useState<string[]>(['companyLogo', 'companyDescription'])
      ...
      <div className="w-full max-w-4xl space-y-6">
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p className="text-xs text-muted-foreground text-center max-w-md">
            You can skip this step and add guest types later, or add them now to get shareable registration links
          </p>
        </div>
