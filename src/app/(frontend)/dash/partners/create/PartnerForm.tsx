...
import { CreatePartnerTypeDrawer } from './CreatePartnerTypeDrawer'
...
export function PartnerForm(props: PartnerFormProps) {
  ...
  const [partnerTypeOptions, setPartnerTypeOptions] = useState<TypeOption[]>(props.partnerTypes)
  ...
  const defaultValues: PartnerFormValues =
    props.mode === 'edit'
      ...
      : {
          ...
          partnerType:
            partnerTypeOptions.length === 1 ? partnerTypeOptions[0].id : (undefined as any),
          contactPerson: '',
          contactEmail: '',
          email: null,
  ...
  const cancelHref =
    props.mode === 'edit' ? `/dash/partners/${props.partnerId}` : '/dash/partners'
  ...
  return (
    <>
      ...
      <CreatePartnerTypeDrawer
        ...
        onCreated={(item) => {
          setPartnerTypeOptions((prev) => [...prev, item])
          setValue('partnerType', item.id)
        }}
      />
    </>
  )
