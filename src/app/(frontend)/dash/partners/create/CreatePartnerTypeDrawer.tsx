...
import { PARTNER_FIELD_OPTIONS } from '@/lib/schemas/partner-type'
import { quickCreatePartnerType } from './actions'
...
export function CreatePartnerTypeDrawer({ open, onOpenChange, onCreated }: Props) {
  const {
    handleSubmit,
    control,
    reset,
  ...
  const onSubmit = async (values: DrawerValues) => {
    const result = await quickCreatePartnerType(values)
    if (result.success) {
    ...
    >
      <SheetContent side="right" className="flex flex-col sm:max-w-md">
        <SheetHeader className="px-6 pt-6">
          <SheetTitle>New Partner Type</SheetTitle>
        </SheetHeader>
        <form
          ...
          <FieldSet>
            ...
            <FieldGroup>
              ...
              <Controller
                ...
                control={control}
                render={({ field, fieldState }) => {
                  const allValues = PARTNER_FIELD_OPTIONS.map((o) => o.value)
                ...
                render={({ field }) => (
                  <Field>
                    <div className="flex items-center gap-2">
                      <Checkbox
                        id="type-showOptional"
                        checked={field.value}
                        onCheckedChange={field.onChange}
                      />
                      <FieldLabel htmlFor="type-showOptional" className="cursor-pointer">
