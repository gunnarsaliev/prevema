...
type Props =
  ...
  | ({ mode: 'edit'; partnerTypeId: string; defaultValues: PartnerTypeFormValues; organizations: OrgOption[]; events: EventOption[] } & SharedCallbacks)

export function PartnerTypeForm(props: Props) {
  const router = useRouter()
  const [serverError, setServerError] = useState<string | null>(null)

  const organizations = props.organizations

  const defaultValues: Partial<PartnerTypeFormValues> =
    props.mode === 'edit'
      ? props.defaultValues
      : {
          organization:
  ...
  const onSubmit = async (values: PartnerTypeFormValues) => {
    setServerError(null)
    try {
      const url =
        props.mode === 'edit'
          ? `/api/partner-types/${props.partnerTypeId}`
          : '/api/partner-types'
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-2xl w-full">
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Textarea
                  ...
                  aria-invalid={fieldState.invalid}
                  className="bg-background"
                  placeholder="Describe this partner type…"
                  rows={2}
                />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              </Field>
                ...
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <div className="flex items-center gap-2 h-9">
                    ...
                    <span className="text-sm text-muted-foreground">
                      This partner type is accepting registrations
                    </span>
                  </div>
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                </Field>
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <p className="text-sm text-muted-foreground">
                  When enabled, partners see these fields but are not required to fill them in.
                </p>
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              </Field>
            )}
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" disabled={isSubmitting}>
          ...
          {props.mode === 'edit' ? 'Save changes' : 'Create partner type'}
        </Button>
        <Button
          type="button"
          variant="outline"
          onClick={() => props.onCancel ? props.onCancel() : router.push('/dash/partner-types')}
