    ...
    <div className="px-6 py-8 max-w-2xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          <Badge variant={STATUS_VARIANT[event.status ?? 'planning']}>{event.status ?? 'planning'}</Badge>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field
          label="Start date"
          value={event.startDate ? format(new Date(event.startDate), 'dd MMM yyyy, HH:mm') : undefined}
        />
        <Field
          label="End date"
