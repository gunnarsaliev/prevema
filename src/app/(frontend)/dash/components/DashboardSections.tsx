...
export async function DashboardStats({ organizationIds }: Props) {
...
export async function UpcomingEventSection({ organizationIds }: Props) {
  const upcomingEvent = await getCachedUpcomingEvent(organizationIds)
...
export async function QuickStartSection({ organizationIds }: Props) {
  const counts = await getCachedDashboardCounts(organizationIds)
            ...
            {counts.events === 0 ? (
              <div className="flex items-center gap-4 rounded-lg border p-4 cursor-not-allowed">
                ...
                <Link href="/dash/events/create" className="shrink-0">
                  <Badge
                    variant="outline"
                    className="gap-1 text-xs text-primary border-primary/40 hover:bg-primary/10 transition-colors cursor-pointer"
                  >
