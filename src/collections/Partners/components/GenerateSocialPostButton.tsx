 ...
 * Displays a button in the partner detail view to generate social media posts for all platforms
 */
export const GenerateSocialPostButton: React.FC = () => {
  ...
  const fieldOfExpertise = useFormFields(([fields]) => fields?.fieldOfExpertise?.value as string)
  const companyDescription = useFormFields(([fields]) => fields?.companyDescription?.value as string)
  const sponsorshipLevel = useFormFields(([fields]) => fields?.sponsorshipLevel?.value as string)
  const contactPerson = useFormFields(([fields]) => fields?.contactPerson?.value as string)
    ...
    <div style={{ marginTop: '1rem' }}>
      ...
      {!id && (
        <p style={{ fontSize: '12px', color: '#666', marginTop: '0.5rem' }}>
          Save the partner first to enable social posts generation
        </p>
      )}
