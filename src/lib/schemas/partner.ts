...
export const partnerSchema = z.object({
  ...
  partnerType: z.number({ required_error: 'Partner type is required' }),
  contactPerson: z.string().min(1, 'Contact person is required'),
  contactEmail: z.string().email('Invalid email address'),
  email: z.string().email('Invalid email address').nullish().or(z.literal('')).transform(v => v || null),
  ...
  status: z.enum(['default', 'contacted', 'confirmed', 'declined']).default('default'),
